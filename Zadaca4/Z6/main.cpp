...
{
    std::map<int,std::shared_ptr<Student>> studenti;//broj indeksa je id
    std::map<int,std::shared_ptr<Laptop>> laptopi;//ev_broj je id
public:
    Administracija() {}
    ...
    void RegistrirajNovogStudenta(int indeks,std::string godina,std::string imeIPrezime,std::string adr,std::string tel)
    {
        ...
        studenti.insert({indeks,std::make_shared<Student>(indeks,godina,imeIPrezime,adr,tel)});
    }
    void RegistrirajNoviLaptop(int broj,std::string n,std::string k)
    {
        ...
        laptopi.insert({broj,std::make_shared<Laptop>(broj,n,k)});
